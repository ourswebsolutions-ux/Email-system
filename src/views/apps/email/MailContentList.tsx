...
const MailContentList = (props: Props) => {
  ...
  const handleEmailClick = (id: number) => {
    ...
    if (store.currentEmailId !== id || emails.find(e => e.id === id)?.isRead === false) {
              ...
              <div className='flex items-center justify-between gap-2'>
                <div className='flex items-center gap-2 overflow-hidden'>
                  ...
                  <IconButton onClick={e => handleToggleStarEmail(e, email.id)}>
                    <i className={classnames('tabler-star', email.isStarred ? 'text-warning' : 'text-textSecondary')} />
                  </IconButton>
                ...
                {!isBelowSmScreen && (
                  <div className={classnames('flex items-center gap-2', styles.emailInfo, { [styles.show]: isBelowLgScreen })}>
                    <div className='flex items-center gap-2'>
                      {email.labels?.map((label) => (
                        <i
                          key={label}
                          className={classnames(
                            'tabler-circle-filled text-[10px]',
                            labelColors[label]?.colorClass || 'text-gray-400' // fallback color
                          )}
                        />
                      ))}
                    </div>
